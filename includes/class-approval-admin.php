...
class ApprovalAdmin
{
        ...
        <div class="wrap approval-plugin-wrap">
            ...
            <div class="approval-page-header">
                ...
                <div class="page-header-right">
                    <a href="<?php echo admin_url('admin-post.php?action=approval_export_csv'); ?>"
                        class="button button-secondary header-action-btn">
                        <span class="dashicons dashicons-download"></span>
                        <?php esc_html_e('Export CSV', 'request-flow-pro'); ?>
                    </a>
                    <a href="<?php echo admin_url('admin.php?page=approval-settings'); ?>"
                        class="button button-secondary header-action-btn">
            ...
            <div class="approval-filters-wrapper">
                <div class="approval-filters-section">
                    <div class="filter-tabs">
                        ...
                        <a href="?page=approval-requests&status=pending"
                        ...
                        <a href="?page=approval-requests&status=approved"
                        ...
                        <a href="?page=approval-requests&status=rejected"
                                    ...
                                    <td>
                                        ...
                                        <div class="row-actions">
                                            <span><a href="#" class="view-details"
                                                    data-id="<?php echo $request->id; ?>"><?php esc_html_e('View Details', 'request-flow-pro'); ?></a></span>
                                        </div>
                                    </td>
                                    <td>
                                        <?php
    ...
    public function settings_page()
    {
        ...
        // Email Templates
        $email_approved_subject = get_option('approval_plugin_email_approved_subject', __('Your request has been approved', 'request-flow-pro'));
        $email_approved_body = get_option('approval_plugin_email_approved_body', $this->get_default_approved_template());
        $email_rejected_subject = get_option('approval_plugin_email_rejected_subject', __('Your request has been rejected', 'request-flow-pro'));
        $email_rejected_body = get_option('approval_plugin_email_rejected_body', $this->get_default_rejected_template());
        $email_pending_subject = get_option('approval_plugin_email_pending_subject', __('Your request has been received', 'request-flow-pro'));
        ...
        $pending_message = get_option('approval_plugin_show_pending_message', __('Your request is pending approval. You will be notified via email once it has been reviewed.', 'request-flow-pro'));
            ...
            <form method="post" action="" class="approval-settings-form">
                ...
                <div class="approval-settings-tabs">
                    <nav class="nav-tab-wrapper">
                        <a href="#general"
                            class="nav-tab nav-tab-active"><?php esc_html_e('General', 'request-flow-pro'); ?></a>
    ...
    public function handle_status_update()
    {
        if (!wp_verify_nonce($_POST['_wpnonce'], 'approval_update_status')) {
            wp_die(__('Security check failed.', 'request-flow-pro'));
        }

        if (!current_user_can('manage_options')) {
            wp_die(__('You do not have permission to perform this action.', 'request-flow-pro'));
        }
    ...
    private function send_status_notification($request_id, $status)
    {
        ...
        if ($request) {
            ...
            if ($status === 'approved') {
                $subject = get_option('approval_plugin_email_approved_subject', __('Your request has been approved', 'request-flow-pro'));
                $body_template = get_option('approval_plugin_email_approved_body', "Hello {user_name},\n\nGreat news! Your request \"{request_title}\" has been approved.\n\n{admin_notes}\n\nThank you,\n{site_name}");
            } else {
                $subject = get_option('approval_plugin_email_rejected_subject', __('Your request has been rejected', 'request-flow-pro'));
    ...
    public function export_csv()
    {
        ...
        if (empty($requests)) {
            wp_die(__('No requests to export.', 'request-flow-pro'));
        }
